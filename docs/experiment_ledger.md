...
| `P3-PHASE-E9A-PREREG-v1` | 2026-07-29 | 3 / PRE-REGISTERED SEQUENTIAL ENGINEERING | raw/normalized × A0/A1；LR `3e-4`；train 排序 9–16；slots `40001..41600`；held-out `75..106` | 预算 800 updates、6,400 train objectives、2,048 held-out objectives；固定 inverse-initial-loss weights；**尚未运行** | 只检验单变量 sample-tail mitigation；不选 step 100、不降低旧门槛。排序 17–28 仅冻结身份，E.9a 不解码/训练；full E、A2/A4/OOD 继续锁定 |
...
| 2026-07-29 / Gate E.9a 预注册 | 只跑 normalized A0/A1 会把新 flow draw 与 weighting 混淆；直接消费剩余 12 条又会失去独立复验 | 同时冻结 raw/normalized × A0/A1 四轨；权重只由 E.8 zero-gate initial loss 决定且和为 8；step 200 与原 absolute/paired Gate 不变 | 新 train/held-out identity、32-comparison FWER 和三种互斥分类已编码；排序 17–28 的 cohort/flow SHA 只登记不读取 | 否；当前是 PRE-REGISTERED/NOT RUN，没有 loss、harm、candidate 或 OOD 结果 |
...
6. 20-step shadow RGB 生成成本是否等于阶段三 cached latent 或在线 Adapter
   成本；阶段三必须单独计时。
7. E.8 mixed 结果能否由 E.9a 固定 sample normalization 缓解，以及候选能否
   推广到 train 排序 17–28。E.9a 目前只有预注册代码与协议，没有真实结果；
   中间 checkpoint 仍不是正式选择，A2/A4 和阶段三成功率继续锁定。
