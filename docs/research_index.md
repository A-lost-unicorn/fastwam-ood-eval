...
| 阶段三：Future-to-Action Adapter | Phase A/B/C/D 完成；E/E.1–E.8 均已真实执行 | E.8 只读 A0 step 100/200、64 个新 flow，共 1,536 objectives；step 200 pooled mean 改善 3.728%，但 full/双 block 仅 4/8、4/8、5/8 stable；严格确认 1 条 target 和 1 条非 target 恶化 | **E.8 工程通过，分类 `mixed_or_inconclusive`**。五-flow 方差与真实 sample-level harm 同时存在；不得选 step 100、扩 A2/A4 或启动 ID/OOD |
...
6. E.8 已按冻结协议完成并得到 `mixed_or_inconclusive`：step 200 pooled mean
   ...
   方差或普遍 tail risk。
7. 不再在同八条 sample 上继续增加 flow。下一步需先预注册 matched A0/A1 的
   单变量 sample-tail mitigation，并保留 train 排序 17–28 做一次性独立复验。
8. 只有后续产生稳定候选配方，并在未使用 cohort 复验及新的完整 28/4 Gate E
   中通过 train/dev、resume、frozen 与尺度门槛后，才允许实现和训练 A2/A4。
9. 当前不得覆盖 E.5/E.6/E.7/E.8 Run ID、事后放宽门槛、启动完整 Gate E、A2/A4、
