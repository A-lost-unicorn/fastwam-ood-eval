...
- 基于诊断实现 `1.335M` 参数 Fast-WAM-GeoEq，并在 `3×RTX 4090` 上完成 B1/G3/G4 单任务 matched Pilot；G3 Camera gap 缩小 `20.94%<25%`、future utility 仍为 `−0.005231`、Camera success 与基线同为 `1/4`，遂停止预计 `28–45 h` 的 formal，并以 CPU-only 只读分解定位 Clean/低 sigma 伤害及 RayPose 因果缺口。

## 30 秒项目叙述

我没有直接把“能生成未来”当成“未来对控制有用”，而是先验证 Clean/OOD 行为缺口，再依次检查 shadow-future 关联、future-content 动作敏感性和 K=0/K=1 held-out utility。K=1 没有正向收益后，我按冻结规则停止 OOD 扩展；随后用 exact-state probe 定位 Camera 等变性缺口，并三卡验证针对性 GeoEq 配方。Pilot 仍未过门，所以我停止 formal，只用已有工件定位下一条 condition-aware/low-noise 假设。
