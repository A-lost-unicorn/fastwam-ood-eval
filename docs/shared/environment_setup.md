...
不要在准备工件后直接用一个脚本连续启动 Clean、OOD 或 full。按 [思考点 1 实施与验收手册](../thought1/execution_guide.md) 分别运行带配置的 doctor、单卡 Clean smoke、单卡 OOD smoke 和三卡 pilot，并在每一阶段检查工件后再继续。当前 checkout 没有 checkpoint/Plus assets 的验证记录；真实接口状态为未验证。
