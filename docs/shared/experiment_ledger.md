...
| `P3-PHASE-E8-v1` | 2026-07-29 | 3 / COMPLETED READ-ONLY DIAGNOSTIC | prereg commit `e6ebdf2`；A0 step 100/200；flows `11..74`；双 32-flow block | 1,536/1,536 forward、0 backward/optimizer/write；step-200 full reduction 3.728% 但 4/8 stable；1 条 target + 1 条非 target 经 FWER 确认恶化；18.51 分钟 | 工程 Gate 通过；冻结分类 `mixed_or_inconclusive`，既不支持 2/3 target tail-risk，也不支持纯 five-flow variance；不选择 checkpoint、不解锁 A2/A4/OOD |
...
| `P3-PHASE-E9A-PREREG-v2` | 2026-07-29 | 3 / PRE-REGISTERED SEQUENTIAL ENGINEERING | 同一 raw/normalized × A0/A1 科学设计；generic positive-integer flow evaluator；新 config/schema/runner/output | `75..106` 完整 8×32 grid/outcome 回归与 initial-probe failed-status 故障注入通过；**本行冻结时尚未运行，后续结果另记** | v1 失败前置校验；排序 17–28 仍 identity-only reserve；full E、A2/A4/OOD 继续锁定 |
...
| `P3-PHASE1-K1-ONLINE-CF-PREREG-v1` | 2026-07-30 | 3 / IMPLEMENTED PRE-REGISTERED ENGINEERING SMOKE | E6 A1@3e-4 step-200；8 条 E6 train cohort；B0/correct/formal-null/shuffle；K=1、20 action steps | config/schema/CLI/runner/checksum-resume/metrics/latency/A-B-C rule 已实现；73 related tests + no-Torch dry-run 通过；**本行冻结时尚未运行，后续结果另记** | 只允许产生 action-sensitivity A/B/C；不读 success/OOD、不 rollout、不自动 Phase 2；禁止多 checkpoint 选择 |
| `P3-PHASE1-K1-ONLINE-CF-v1` | 2026-07-30 | 3 / VALID ENGINEERING SMOKE | prereg commit `f516920`；GPU 1；E6 A1@3e-4 step-200；8 条同 task train sample；B0/correct/formal-null/shuffle | B0 replay 与 B0-null L2/L∞ 全 0；correct-null、correct-shuffle、action-hash 均 `8/8`；L2 mean `0.011052/0.012092`；paired K1 overhead `258.95 ms` mean；62/62 artifact SHA、frozen/no-grad/no-cache/no-RGB checks 通过 | 分支 A `future_content_sensitivity_observed`；只支持 fixed-checkpoint future-content action sensitivity。Phase 2 设计解锁但未启动；success/OOD/K 排序仍不支持 |
...
- 权威工件：
  ...
  详细解释见 [thought1_report.md](../thought1/report.md)。
...
- 权威人工解读见 [thought2_formal_results.md](../thought2/formal_results.md)；
  机器工件位于
...
- 完整解释见 [thought3_phase_c_report.md](../thought3/phase_b_d/phase_c_report.md)。
...
- optimizer 未创建、backward/optimizer step 为 0、训练未启动。完整解释见
  [thought3_phase_d_report.md](../thought3/phase_b_d/phase_d_report.md)。
...
- Gate 在 frozen-after hash 前停止，因此该项仍未闭环。完整失败边界与下一步见
  [thought3_phase_e_report.md](../thought3/gate_e/phase_e_report.md)。
...
| 2026-07-29 / Gate E.8 结果 | 64-flow pooled mean 改善，但 step-200 full/双 block 的逐样本 Gate 仍失败；三条 target 只有一条确认，另有一条非 target 确认恶化 | flow panel 方差和真实 sample-level heterogeneity 同时存在；原点符号 6/8 Gate 对小幅不确定变化敏感，但 `episode_000012` 的 +8.881% harm 跨 block 与校正 CI 稳定 | 保留 `mixed_or_inconclusive`，不降低 2/3、不换 target、不继续堆同 cohort flow；下一协议应把 pooled、point-sign 与 confirmed harm 分层并做 matched tail mitigation | 否；1,536 objectives 完整，0 backward/optimizer/write，全部 frozen/RNG/provenance/leakage checks 通过 |
...
| 2026-07-30 / Phase 1 K=1 online CF 结果 | 需区分 future latent 的具体内容、仅 latent presence、hook 开销和 B0 数值噪声 | B0×2 与 formal null 精确相同；correct/shuffle 共用 target context/action seed/recipient future seed，donor 仅来自其他 episode；八条 current-only sample 均完成 | 冻结分类为分支 A；下一步只预注册一次 28/4 normalized matched A0/A1 Phase 2，不自动训练、不改 checkpoint/LR/门槛 | 否；`8/8` correct-null、`8/8` correct-shuffle、`8/8` action-hash 证明技术内容敏感性，但 action RMS 仅约 B0 的 2.2%–2.4%，无 rollout/success/OOD/K 排序 |
| 2026-07-30 / Phase 2 full 28/4 预注册实现 | 需要把 Branch A 转成完整训练候选，同时避免再次做 LR/weight/checkpoint surrogate sweep | E9a-v2.1 audit valid 后按冻结规则只选 normalized recipe；单卡一次性生成 28-sample weights，A0/A1 两卡并行，共用 200×28 schedule；dev 只评 step 0/200 | config/schema/CLI、calibration、Adapter-only resume、双卡 launcher、CPU finalize、58 项定向测试和四 stage dry-run 已完成；fingerprint `fabb96a...468` | 否；状态 `PRE-REGISTERED/NOT RUN`，无 Phase 2 loss/checkpoint/OOD/success。真实运行仍需 `CONFIRM_THOUGHT3_PHASE2_FULL=YES` |
| 2026-07-30 / Phase 2 calibration manifest 中断 | 冻结 commit `96ace8b` 完成 28×32 train 与 4×32 dev calibration 后，`calibration.json` 已写、artifact manifest 尚未写时命令退出 | config output 为相对路径，safety root 为绝对路径；旧 manifest writer 直接 `relative_to`，触发 representation-only `ValueError` | 保留 896+128 rows；train/dev row SHA `0f812c75...4b997` / `07f29ba3...3a5d`，weight SHA `4c36dece...1dc22`；统一 resolve path/root 并增加 mixed-path/outside-root 回归；只允许原目录 `--resume` | 否；A0/A1 目录、optimizer update、backward、新 checkpoint 均为 0。不是 loss/GPU/data 失败，不改变任何科学配方或门槛 |
| 2026-07-30 / Phase 2 full 28/4 结果 | 修复 commit `8e41d0b` 后原目录 resume；A0/A1 各 200×28 training objectives，dev 固定 step 0/200 | 12/12 hard checks；32/32 manifest descriptors；8/8 checkpoint provenance；A0 reduction `+1.845%`，A1 `−1.712%`，A1 final 比 A0 高 `3.624%` 且 4/4 sample 更差 | 冻结分类 `training_valid_dev_direction_not_observed`；登记 future sensitivity 未转化为 offline utility | 否；有效负结果。`phase3_unlocked=false`，停止完整-checkpoint CF、OOD pilot、A2/A4 和 outcome-driven 调参 |
...
| 2026-08-01 / Thought4 smoke v6 | canonical JSON 修复后需完整复验四条件真实链路 | commit `aeb02106`、GPU 2；2 states×4 conditions、80 features；model load 409.889 s；backbone SHA 前后 `ac0dd59...b4f8`；identity replacement L2=0 | 登记 `PASSED / NON-SCIENTIFIC`，result SHA `b260977a...4c37`；只解锁同 commit formal v4 | 否；技术 Gate，不是 probe/method/OOD/success 结果 |
| 2026-08-01 / Thought4 formal v4 | 第 2 个 state `episode_000031@t34` 在 prefix-to-parquet alignment 报 0.031214 m / 2.153° | pre-model fail closed；0 paired manifest/feature/probe/intervention。随后对原 64 states 只读审计：56 pass、8 fail；train/dev/test 失败 5/3/0；translation max 0.108324 m、rotation max 28.918° | 冻结 v4 error；不 resume、不放宽 3 cm / 15°、不删/替换 state。alignment 是 replay-fidelity QC，不是 policy outcome | 否；没有科学结果，审计未读取模型/future RGB/success/OOD |
| 2026-08-03 / Thought4 simulator-replay v5 预注册 | parquet EEF 与真实 prefix state 存在 8/64 尾部失配，直接 hard-gate 会选择性丢失冻结 cohort | 保留原 64 identity 与原阈值；alignment 改完整 disclosure；从实际 input `t` replay `a_t...a_{t+H-1}`；exact 三条件共享 Clean world trajectory，Robot-init 独立 replay；新 alignment artifact+SHA gate | 新 smoke v7/formal v5；旧 v6 PASS 不复用；先 smoke 后 formal；当前 `PRE-REGISTERED / NOT RUN` | 否；单变量标签源工程修复，不读 future RGB/success/OOD/policy outcome，不改变 probe/layer/seed/threshold |
...
25. Gate E.8 用 64 个全新 flow 和两个独立 32-flow block 完成 1,536 个只读
    objective。A0 step 200 pooled mean 在 full/双 block 均改善，但原逐样本
    Gate 分别只有 4/8、4/8、5/8；`episode_000012` 的 +8.881% target harm 和
...
27. Phase 1 已将原始 Fast-WAM B0 replay、online K=1 correct、无 tensor
    ...
    cohort、seed、指标和 A/B/C 决策。真实单卡结果中 B0/null 精确一致，
    correct-null、correct-shuffle 与 action-hash 均为 8/8，按预注册规则进入
    `future_content_sensitivity_observed`。该证据只覆盖一个 checkpoint、一个
    task、八条 train sample 的技术动作敏感性。
...
6. 20-step shadow RGB 生成成本是否等于阶段三 cached latent 或在线 Adapter
   成本；阶段三必须单独计时。
7. Phase 1 观察到的约 `0.011–0.012` normalized action RMS 差异是否会转化成
   轨迹或 success 改变；必须由完整 28/4 A0/A1 checkpoint 复验和后续 paired
   rollout 回答。A2/A4 与正式 OOD 仍锁定。
...
| `C3a` | 固定 E6 A1 checkpoint 的 K=1 future latent 具体内容会改变动作 | 3 / technical | B0 replay、formal-null parity、paired correct/shuffle、frozen/no-cache/no-leakage audit | **支持（SMOKE；单 task 八条 train sample）** | `P3-PHASE1-K1-ONLINE-CF-v1` |
| `C3b` | K=1 动作敏感性在冻结轻量 Adapter 配方中形成 held-out action-objective 收益 | 3 / offline utility | matched 28/4、固定 step-200、A0/A1 同 schedule/weight、完整 hard checks | **本配方不支持（VALID OFFLINE NEGATIVE）** | `P3-PHASE2-FULL-28-4-v1`；`outputs/thought3/phase2_full_28_4_a0_a1_v1/` |
