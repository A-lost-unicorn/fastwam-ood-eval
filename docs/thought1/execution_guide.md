...
- [正式报告](report.md)：完整分母、正式结果、分层统计、完整性审计和结论边界。
- [研究结论边界](generalization.md)：本阶段能回答和不能回答什么。
- [完成度审计](readiness.md)：正式完成证据和仍限制机制解释的缺口。
- [环境配置](../shared/environment_setup.md)：Python、PyTorch、CUDA、MuJoCo/EGL 兼容方案。
- [实验协议](../shared/experiment_protocol.md)：配对、统计和成功率口径。
- [上游勘察](../shared/upstream_notes.md)：锁定的上游 commit、真实 API 与许可证。
