...
[静态/无动作校准手册](static_calibration.md)。
...
[盲审与 outcome-blind 抽样手册](blind_review_and_sampling.md)。
...
及 outcome 最小分母见
[统计分析计划](statistical_analysis_plan.md)。该计划在正式
...
人工解释见 [thought2_formal_results.md](formal_results.md)，机器表位于
...
详细机器结果位于 `outputs/thought2_unconditional_smoke/summary/`，证据解释见 [experiment_ledger.md](../shared/experiment_ledger.md)。
