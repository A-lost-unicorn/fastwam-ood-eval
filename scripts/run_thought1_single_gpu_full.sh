...
mkdir -p "${lock_dir}"
lock_file="${lock_dir}/.single_gpu_full.lock"
exec 9>"${lock_file}"
if ! flock -n 9; then
  echo "Another single-GPU Thought 1 full script holds ${lock_file}." >&2
  echo "Do not run two evaluators against the same output directories." >&2
  exit 1
fi
