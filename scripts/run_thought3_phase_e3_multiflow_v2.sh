...
if [[ "${CONFIRM_THOUGHT3_PHASE_E3_V2:-}" != "YES" ]]; then
  echo "Refusing real Gate E.3 v2: set CONFIRM_THOUGHT3_PHASE_E3_V2=YES" >&2
  exit 2
fi
...
if (( used_mib > 1024 )); then
  echo "GPU ${physical_gpu_id} is not an idle Gate E.3 v2 window (${used_mib} MiB used)" >&2
  exit 2
fi

export CUDA_VISIBLE_DEVICES="${physical_gpu_id}"
export THOUGHT3_PHYSICAL_GPU_ID="${physical_gpu_id}"
export HF_DATASETS_CACHE="${THOUGHT3_HF_DATASETS_CACHE:-/tmp/thought3_phase_e3_v2_hf_cache}"
...
log_dir="${project_root}/outputs/thought3/phase_e3_multiflow_v2/logs"
mkdir -p "${log_dir}"
