...
if [[ "${CONFIRM_THOUGHT3_PHASE_E9A:-}" != "YES" ]]; then
  echo "Refusing real Gate E.9a: set CONFIRM_THOUGHT3_PHASE_E9A=YES" >&2
  exit 2
fi
...
if [[ "${physical_gpu_id}" == *,* ]]; then
  echo "Gate E.9a is single-GPU only; provide one THOUGHT3_GPU_ID" >&2
  exit 2
fi
used_mib="$(
  nvidia-smi \
...
if (( used_mib > 1024 )); then
  echo "GPU ${physical_gpu_id} is not idle for E.9a (${used_mib} MiB used)" >&2
  exit 2
fi

export CUDA_VISIBLE_DEVICES="${physical_gpu_id}"
export THOUGHT3_PHYSICAL_GPU_ID="${physical_gpu_id}"
export HF_DATASETS_CACHE="${THOUGHT3_HF_DATASETS_CACHE:-/tmp/thought3_phase_e9_hf_cache}"
...
log_dir="${project_root}/outputs/thought3/phase_e9_sample_tail_mitigation_v1/logs"
mkdir -p "${log_dir}"
log_path="${log_dir}/phase_e9a.log"
