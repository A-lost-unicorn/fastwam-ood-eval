...
if [[ "${CONFIRM_THOUGHT3_PHASE_E:-}" != "YES" ]]; then
  echo "Refusing real Phase E: set CONFIRM_THOUGHT3_PHASE_E=YES" >&2
  exit 2
fi
...
if (( used_mib > 1024 )); then
  echo "GPU ${physical_gpu_id} is not an idle Phase E window (${used_mib} MiB used)" >&2
  exit 2
fi
...
"${project_root}/.conda/envs/fastwam-ood/bin/python" -m fastwam_ood_eval.cli \
  ...
  --config configs/thought3/phase_e_training_smoke.yaml \
  --device cuda:0 \
  "$@" 2>&1 | tee -a "${log_path}"
