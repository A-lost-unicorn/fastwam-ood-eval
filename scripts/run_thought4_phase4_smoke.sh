...
if [[ "${CONFIRM_THOUGHT4_PHASE4_SMOKE:-}" != "YES" ]]; then
  echo "Refusing real Thought4 smoke: set CONFIRM_THOUGHT4_PHASE4_SMOKE=YES" >&2
  exit 2
fi
...
if [[ -n "$(git status --porcelain --untracked-files=all)" ]]; then
  echo "Refusing real Thought4 smoke: commit a clean project snapshot first" >&2
  exit 2
fi

status_path="${project_root}/outputs/thought4/phase4_geometry_action_smoke_v7/run_status.json"
...
output_root="${project_root}/outputs/thought4/phase4_geometry_action_smoke_v7"
