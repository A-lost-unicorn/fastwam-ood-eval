...
def test_all_committed_thought3_configs_validate():
    ...
    assert (
        Path(
            "configs/thought3/"
            "phase_e4_diversified_flow_diagnostic.yaml"
        )
        in paths
    )
    assert (
        Path(
            "configs/thought3/"
            "phase_e5_objective_aggregation_diagnostic.yaml"
        )
        in paths
    )
    assert (
        Path(
            "configs/thought3/"
            "phase_e6_fresh_cohort_replication.yaml"
        )
        in paths
    )
    assert (
        Path(
            "configs/thought3/"
            "phase_e7_checkpoint_trajectory.yaml"
        )
        in paths
    )
    assert (
        Path(
            "configs/thought3/"
            "phase_e8_a0_flow_variance_replication.yaml"
        )
        in paths
    )
    assert (
        Path(
            "configs/thought3/"
            "phase_e9_sample_tail_mitigation_v2.yaml"
        )
        in paths
    )
    phase2 = Path(
